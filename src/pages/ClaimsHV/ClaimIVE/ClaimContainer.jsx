...
import { CLAIM_IVE, COMMENT_IVE } from "constant/endpoints";
import {
  Dialog,
  DialogTitle,
  DialogContent,
...
function ClaimContainer() {
  ...
  const handleAddComment = (comment) => {
    ...
    api(COMMENT_IVE + "/" + id, { method: "PATCH", body: newComment }).then(
      ({ ok, body }) => {
        if (ok) {
          navigate(0);
        }
  ...
  const handleOnSubmit = () => {
    ...
    api(CLAIM_IVE+"/"+id,{method: "PATCH", body: values}).then(
      ({ ok, body }) => {
        if (ok) {
