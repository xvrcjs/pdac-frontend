...
const DataGrid = ({ columns, rows, pageSize = 10, handleEdit,hasFilter,noDataMessage,backgroundColor, handleDelete,setTicketSelected,setShowAssignTicket,setShowMessageConfirmReAssign,setShowMessageConfirmReOpen,account}) => {
  ...
  const handleReAssignTicket = (ticket) =>{
    setTicketSelected(ticket.id)
  ...
  const handleReOpenTicket = (ticket) =>{
    setTicketSelected(ticket.id)
    setShowMessageConfirmReOpen(true)
  }
                  ...
                  >
                      {account.roles[0].name === "Admin" && (
                        ...
                        {row.status.data === "Cerrado" ? (
                          ...
                          row.assigned === null ? (
                            <img src="../../assets/claims/table/paper_plane.svg" alt="asignar-reclamo" onClick={() => handleAssignTicket(row)} style={{cursor:"pointer",marginRight:"40px"}}/>
                            ):(
                            <img src="../../assets/claims/table/forward_circle.svg" alt="re-asignar-reclamo" onClick={() => handleReAssignTicket(row)} style={{cursor:"pointer",marginRight:"40px"}}/>)
                        )}
                        </>)
                      }
